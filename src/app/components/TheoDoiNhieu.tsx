/* eslint-disable @next/next/no-img-element */
import React from 'react';

const TheoDoiNhieu = () => {
    return (
        <div className='lg:w-356 lg:h-374 py-6 flex h-auto w-full flex-col items-center lg:items-start'>
            <div className='lg:w-356 h-30 relative flex w-full flex-row items-start justify-start'>
                <div className='h-30 w-20 bg-gray-300'>
                    <div className='break-words py-2 text-center text-sm font-normal text-black  md:text-base'>
                        Theo Dõi
                    </div>
                </div>
                <div className='break-words px-2 py-2 text-sm font-bold uppercase text-black underline  md:text-base'>
                    Nhiều
                </div>
            </div>
            <div className='lg:w-355 h-auto mt-2 flex w-full gap-3'>
                <div className='w-282 h-20 flex-1 flex-col items-start justify-start gap-3'>
                    <div className='flex flex-row '>
                        <div className='w-31 h-31 md:w-10 md:h-8 lg:w-8 flex items-center justify-center rounded-full bg-gray-300 px-3 py-0'>
                            <div className='text-sm font-normal uppercase text-black  md:text-base'>1</div>
                        </div>
                        <div className='ml-2'>
                            <div className='text-xs font-bold uppercase text-black   md:text-base'>
                                Bạn Bị bỏ rơi hãy liên hệ với chúng tôi
                            </div>
                        </div>
                    </div>
                    <div className='ml-10  mt-2 text-xs font-normal lowercase text-black  md:text-base'>11,530 theo dõi</div>
                </div>
                <div className='w-24 h-full bg-gray-300  lg:w-28'>
                    <img
                        src='https://m.media-amazon.com/images/M/MV5BNDNiOWM5NGItNzY4NC00MDg1LTljZjctYzViNmRlOTNhOWM2XkEyXkFqcGdeQXVyNjc3OTE4Nzk@._V1_FMjpg_UX1000_.jpg'
                        alt='thumb'
                        className='object-cover h-full w-full'
                    />
                </div>
            </div>
            <div className='lg:w-355 h-auto mt-2 flex w-full gap-3'>
                <div className='w-282 h-20 flex-1 flex-col items-start justify-start gap-3'>
                    <div className='flex flex-row '>
                        <div className='w-31 h-31 md:w-10 md:h-8 lg:w-8 flex items-center justify-center rounded-full bg-gray-300 px-3 py-0'>
                            <div className='text-sm font-normal uppercase text-black  md:text-base'>1</div>
                        </div>
                        <div className='ml-2'>
                            <div className='text-xs font-bold uppercase text-black   md:text-base'>
                                Bạn Bị bỏ rơi hãy liên hệ với chúng tôi
                            </div>
                        </div>
                    </div>
                    <div className='ml-10  mt-2 text-xs font-normal lowercase text-black  md:text-base'>11,530 theo dõi</div>
                </div>
                <div className='w-24 h-full bg-gray-300  lg:w-28'>
                    <img
                        src='https://m.media-amazon.com/images/M/MV5BNDNiOWM5NGItNzY4NC00MDg1LTljZjctYzViNmRlOTNhOWM2XkEyXkFqcGdeQXVyNjc3OTE4Nzk@._V1_FMjpg_UX1000_.jpg'
                        alt='thumb'
                        className='object-cover h-full w-full'
                    />
                </div>
            </div>





        </div>
    );
};

export default TheoDoiNhieu;
